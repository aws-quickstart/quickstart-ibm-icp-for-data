...
from yapl.Exceptions import InvalidParameterException
...
class ConfigureEFS(object):
  ...
  def getEFSParameters(self):
    ...
    result = {}
    params = {}
    list = StackParameters.get("AnsibleAdditionalEnvironmentVariables").split(",")
    ...
    for name in EFSVariableParameterNames:
      ...
      result[varName] = params.get(name)
    #endFor
    return result
  #endDef
  ...
  def configureEFSProvisioner(self):
    """
      ...
      The oc command is used do the configuration.  It is assumed that
      oc has been configured to run with a permanent config context, 
  ...
  def configureEFS(self):
    ...
    TR.info("configureEFS","Executing: ansible-playbook %s, --extra-vars @%s --inventory %s." % (self.playbookPath,self.varFilePath,"/etc/ansible/hosts"))
