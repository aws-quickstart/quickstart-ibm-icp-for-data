...
function usage {
  echo "Usage: run-inception.sh [options]"
...
info $LINENO "Running the ICP inception install."
docker run -v $(pwd):/data -e LICENSE=accept ibmcom/icp-inception:2.1.0.3-ee install -v | tee -a $LOGFILE
