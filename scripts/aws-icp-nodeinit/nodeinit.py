...
class NodeInit(object):
  """
    ...
    that are part of the IBM Cloud Pak cluster installation/deployment on AWS.
  """
  ...
  def getSSMParameterValue(self,parameterKey,expectedValue=None,count=GetParameterMaxTryCount):
    """
    ...
    tryCount = 1
    gotit = False
    while (not gotit and tryCount <= count):
  ...
  def getExecutionStatus(self, commandId, instanceId,expectedValue="Success"):

    """
    This method is used to get the status of sendcommand api. Send Command is used to execute
    remote scripts in boot node and we need to validate the status of execution. If Success 
    the execution of remote script is successful and we can proceed to next steps.
    """
    ...
    while (not gotit and tryCount <= count):
      try:
        if (TR.isLoggable(Level.FINE)):
          TR.fine(methodName, "Get SSM command Status for commandID : %s against instaceId %s" %(commandId,instanceId))
        response = self.ssm.get_command_invocation(CommandId=commandId,InstanceId=instanceId)
        if (not response):
          if (TR.isLoggable(Level.FINEST)):
            TR.finest(methodName, "Failed to get a response for SSM get_command_invocation(): %s" % commandID)
          #endIf
        else:
          if (TR.isLoggable(Level.FINEST)):
            TR.finest(methodName,"Response: %s" % response)
          ...
          parameter = response.get('Status')
          if (parameter == expectedValue):
            gotit = True
          else:
            if (TR.isLoggable(Level.FINER)):
              TR.finer(methodName,"For CommandId: %s ignoring value: %s waiting on value: %s" % (commandId,parameter,expectedValue))
            #endIf
          #endIf
        #endIf
      except ClientError as e:
        TR.error(methodName,"Exception while executing get_command_invocation %s "%e)  
      tryCount += 1    
    #endwhile
    return parameter    
  #endDef    
  ...
  def checkstackStatus(self):
    """
      This method will retrieve stack status from ssmparameters to determine if  bootnode script needs to be run before proceeding ahead
      if stack status is set to Created then  we need bootnode script to be run using ssm command
    """
    methodName = "checkstackStatus"
    self.isUpgrade = False
    try:
      if (TR.isLoggable(Level.FINE)):
        TR.fine(methodName, "Get SSM parameter value : /%s/stackStatus if exists" %(self.stackName))
      self.stackStatus = self.getSSMParameterValue("/%s/stackStatus" %(self.stackName),count=5)
      TR.info(methodName,"status  returned  %s" %self.stackStatus)
      if (self.stackStatus == 'Created'):
        #worker node count has changed we need to run ssm command to invoke add node script 
        # to add ssh and execute docker command from boot node.
        self.isUpgrade = True
        self.bootInstanceId = self.getSSMParameterValue("/%s/bootInstanceId" % self.stackName,count=5)
        TR.info(methodName,"bootInstanceId returned : %s" %self.bootInstanceId)
        TR.info(methodName,"Execute publishKey script in bootnode")
        response = self.ssm.send_command(Targets=[{"Key":"instanceids","Values":[self.bootInstanceId]}],
                              DocumentName="AWS-RunShellScript",
                              Parameters={"commands":["python publishKey.py"],
                                         "executionTimeout":["600"],
                                         "workingDirectory":["/root"]},
                              Comment="Execute script in bootnode to add newly inducted node to cluster",
                              TimeoutSeconds=600)
        time.sleep(GetParameterSleepTime*2)
        ...
        TR.info(methodName,"Executed publishKey script in bootnode")                              
    except ICPInstallationException as e:
      if (TR.isLoggable(Level.FINE)):
        TR.fine(methodName, "SSM parameter value : /%s/stackStatus not found" %(self.stackName))
  ...
  def addNodeToCluster(self):
    ...
    if(gotit):
      ...
      call('./icp-install-docker.bin --install | tee /root/logs/install-docker.log',shell=True)
      os.chdir(self.home)
      # call ssm send command to execute docker run
      TR.info(methodName,"Execute docker Run script in bootnode")
      response = self.ssm.send_command(Targets=[{"Key":"instanceids","Values":[self.bootInstanceId]}],
                            DocumentName="AWS-RunShellScript",
                            Parameters={"commands":["python configureNode.py %s %s %s" %(workerNodeIP,self.ICPVersion, self.role)],
      ...
      if(response=="Success"):                               
        if (TR.isLoggable(Level.FINE)):
          TR.fine(methodName, "Executed docker Run script in bootnode successfully")
      else:
        TR.fine(methodName, "Executed docker Run script in bootnode failed with error %s" %response.get("StandardErrorContent"))
      parameterKey = "/%s/boot-public-key" % self.stackName
      TR.fine(methodName, "delete ssm parameter key %s post setup" %parameterKey)
  ...
  def main(self,argv):
    ...
    try:
      ...
      # need to check for worker node count in each run to determine if this is upgrade
      # or initial run and then call bootnode scripts accordingly
      # upgrade requires ssm commands to be executed on boot

      self.checkstackStatus()
      ...
      # sharath add check for stackName/configureNodeStatus value before executing command  
      
      
      if(self.role != 'master' and self.isUpgrade == True):
